...
export default function HRPayrollDetails() {
  ...
  const { employeeId, emiratesId } = useParams<{ employeeId: string; emiratesId: string }>();
  const { role } = useAuth();
  ...
  const [editField, setEditField] = React.useState<'other' | 'loan' | 'loanRecovery' | 'salaryPaid' | 'notes' | null>(null);
  const [editMonthKey, setEditMonthKey] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const perMonth = new Map<string, MonthRow>();
        ...
        if (employeeId) {
          const broadAttRes = await fetchAttendance({ page: 1, pageSize: 2000, employeeId, token } as any);
          const broadAtt = (broadAttRes.data || []) as AttendanceDto[];
          ...
          for (const key of monthKeys) {
            ...
            const scoped = (scopedRes.data || []) as AttendanceDto[];

            // Index by date (YYYY-MM-DD), with Emirates ID safety check
            const byDate = new Map<string, any>();
            for (const a of scoped as any[]) {
              const recEmiratesId = (a as any).emiratesId ?? (a as any).employee?.emiratesId;
              if (targetEmiratesId && recEmiratesId && String(recEmiratesId) !== String(targetEmiratesId)) continue;

              const rawDate = (a as any).date ?? (a as any).check_in ?? (a as any).checkIn;
              if (!rawDate) continue;
              const dStr = typeof rawDate === 'string' ? rawDate.slice(0, 10) : '';
              if (dStr.slice(0, 7) !== monthIso) continue;
            ...
            for (let d = 1; d <= endDate.getDate(); d++) {
              ...
              if (a) {
                ...
                if (statusRaw === 'P' || statusRaw === 'PRESENT') present += 1;
                ...
                const otVal = (a as any).otHours;
                const parsedOt = typeof otVal === 'number' ? otVal : Number(otVal || 0);
                if (!Number.isNaN(parsedOt)) ot += parsedOt;
              } else {
        ...
        if (employeeId) {
          try {
            ...
            const payrollRows: PayrollDto[] = payrollRes.data || [];
            for (const p of payrollRows) {
              if (!p.month) continue;
              // Validate by Emirates ID when joined employee is present
              const joinedEid = (p as any).employee?.emiratesId as string | undefined;
              if (joinedEid && emiratesId && joinedEid.trim() !== emiratesId.trim()) continue;
              ...
              if (!p.deductions_json) continue;
              let extra: any = p.deductions_json;
  ...
  function isRowEditable(row: MonthRow): boolean {
  ...
  function openEdit(field: 'other' | 'loan' | 'loanRecovery' | 'salaryPaid' | 'notes', row: MonthRow) {
  ...
  async function saveEdit() {
    ...
    const parsedValue =
      editField === 'notes' ? editValue : Number(editValue || 0);

    // Update local state first and capture the updated row for this month
    let updatedRow: MonthRow | null = null;
    setRows((prev) =>
      prev.map((row) => {
        if (row.key !== monthKey) return row;
        let next: MonthRow;
        if (editField === 'notes') next = { ...row, notes: String(parsedValue) };
        else if (editField === 'other') next = { ...row, other: Number(parsedValue) };
        else if (editField === 'loan') next = { ...row, loan: Number(parsedValue) };
        else if (editField === 'loanRecovery') next = { ...row, loanRecovery: Number(parsedValue) };
        else next = { ...row, salaryPaid: Number(parsedValue) };
        updatedRow = next;
        return next;
      }),
    );
    ...
    try {
      ...
      const payloadExtras = { ...effective } as any;
      if (editField === 'notes') payloadExtras.notes = String(parsedValue);
      else if (editField === 'other') payloadExtras.other = Number(parsedValue);
      else if (editField === 'loan') payloadExtras.loan = Number(parsedValue);
      else if (editField === 'loanRecovery') payloadExtras.loanRecovery = Number(parsedValue);
      else payloadExtras.salaryPaid = Number(parsedValue);
      ...
      const existingRow = (existing.data || [])[0] as PayrollDto | undefined;
  ...
  const columns: Column<MonthRow>[] = [
      ...
      render: (r) => (
        isRowEditable(r) ? (
          <button
            ...
            onClick={() => openEdit('other', r)}
          >
            {r.other ? r.other.toFixed(2) : '0.00'}
          </button>
        ) : (
          <span className="text-sm text-[color:var(--text-secondary)] opacity-70 cursor-not-allowed">
            {r.other ? r.other.toFixed(2) : '0.00'}
          </span>
        )
      ),
    },
    {
      ...
      render: (r) => {
        ...
        return <span className={colorClass}>{prevGross.toFixed(2)}</span>;
      },
    },
    {
      key: 'total',
      ...
      render: (r) => {
        ...
        const prevGross = grossByKey.get(prevKey) ?? 0;
        const total =
          (r.present + r.halfDayValue + r.paidLeave + r.ot) * r.daySalary + (r.other + prevGross);
      ...
      render: (r) => (
        isRowEditable(r) ? (
          <button
            type="button"
            className="text-left text-[color:var(--accent)] hover:underline"
            onClick={() => openEdit('salaryPaid', r)}
          >
            {r.salaryPaid ? r.salaryPaid.toFixed(2) : '0.00'}
          </button>
        ) : (
          <span className="text-sm text-[color:var(--text-secondary)] opacity-70 cursor-not-allowed">
            {r.salaryPaid ? r.salaryPaid.toFixed(2) : '0.00'}
      ...
      render: (r) => (
        isRowEditable(r) ? (
          <button
            type="button"
            className="text-left text-[color:var(--accent)] hover:underline"
            onClick={() => openEdit('loan', r)}
          >
            {r.loan ? r.loan.toFixed(2) : '0.00'}
          </button>
        ) : (
          <span className="text-sm text-[color:var(--text-secondary)] opacity-70 cursor-not-allowed">
            {r.loan ? r.loan.toFixed(2) : '0.00'}
    ...
    },
    {
      ...
      render: (r) => (
        isRowEditable(r) ? (
          <button
            type="button"
            className="text-left text-[color:var(--accent)] hover:underline max-w-xs truncate"
            onClick={() => openEdit('notes', r)}
          >
            {r.notes || '—'}
          </button>
        ) : (
          <span className="text-sm text-[color:var(--text-secondary)] opacity-70 max-w-xs truncate cursor-not-allowed">
            {r.notes || '—'}
          </span>
        )
      ),
  ...
  return (
    <IonPage>
      ...
      <IonContent className="p-8 bg-[color:var(--bg)] space-y-6">
        ...
        <div className="zynq-muted text-sm">Home &gt; HR &gt; Payroll &gt; Details</div>
        <div>
          <Button
            variant="secondary"
            size="sm"
          ...
          <>
            {employee && (
              <div className="mt-4 space-y-1 text-sm">
                <div><strong>Company:</strong> {employee.company}</div>
                <div><strong>Employee Name:</strong> {employee.employeeName}</div>
                <div><strong>Emirates ID:</strong> {employee.emiratesId}</div>
                <div><strong>Salary:</strong> {employee.salary.toLocaleString()}</div>
              </div>
            )}
            <div className="mt-6">
              <Table columns={columns} data={rows} emptyText="No payroll data available" />
            </div>
          </>
        )}
        ...
        <Modal
          ...
          footer={(
            <>
              <Button variant="secondary" size="sm" onClick={() => setEditOpen(false)} disabled={saving}>
                Cancel
              </Button>
              <Button size="sm" onClick={saveEdit} disabled={saving || !editField || !editMonthKey}>
                {saving ? 'Saving...' : 'Save'}
              </Button>
            </>
          )}
        >
          {editField && (
            <div className="space-y-3 mt-1">
              <Input
                type={editField === 'notes' ? 'text' : 'number'}
                value={editValue}
