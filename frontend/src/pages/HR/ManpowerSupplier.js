...
import { fetchVendors, createVendor, updateVendor, deleteVendor } from '../../api/vendorsService';
...
export default function HRAttendanceManpowerSupplier() {
        ...
        (async () => {
            try {
                ...
                for (const r of records) {
                    ...
                    if (!vendorId)
                        continue;
                    const vendor = (res || []).find((v) => v.id === vendorId);
    ...
    const columns = [
        ...
        },
        {
            key: 'actions',
            header: 'Actions',
            render: (row) => (_jsx("div", { className: "flex gap-2", children: !isTeamLeader && (_jsxs(_Fragment, { children: [_jsx(Button, { size: "sm", variant: "secondary", onClick: () => {
                                setEditing(row);
                                ...
                                setErrors({});
                                setAddOpen(true);
                            }, children: "Edit" }), _jsx(Button, { size: "sm", variant: "danger", onClick: async () => {
                                if (busy)
                                    return;
    ...
    async function handleCreateSupplier() {
        ...
        try {
            const combinedContact = formPhone.trim()
                ...
                : formContact.trim();
            if (editing) {
                const dto = await updateVendor({ id: editing.id, token }, { name: formName.trim(), contact: combinedContact });
                const updated = {
                    id: dto.id,
                    name: dto.name,
    ...
    return (_jsxs(IonPage, { children: [_jsx(Nav, {}), _jsxs(IonContent, { className: "px-4 py-8 sm:px-6 lg:px-8 bg-[color:var(--bg)] space-y-4", children: [_jsx("div", { className: "text-lg font-semibold", children: "HR / Attendance / Manpower Supplier" }), _jsx("div", { className: "zynq-muted text-sm", children: "Home > HR > Attendance > Manpower Supplier" }), _jsx("div", { children: _jsxs(Button, { variant: "secondary", size: "sm", className: "mt-2 flex items-center gap-2", onClick: () => navigate('/hr/attendance'), children: [_jsx(IonIcon, { icon: chevronBackOutline }), _jsx("span", { children: "Back" })] }) }), _jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-6", children: [_jsxs("div", { className: "flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3", children: [_jsx("div", { className: "text-base font-semibold", children: "Manpower Suppliers" }), _jsx(Button, { size: "sm", variant: "secondary", className: "text-xs sm:text-sm px-3 py-1.5 sm:px-4 sm:py-2", onClick: () => navigate('/hr/attendance/manpower-supplier/add-records'), children: "Add Records" })] }), !isTeamLeader && (_jsx(Button, { size: "sm", className: "text-xs sm:text-sm px-3 py-1.5 sm:px-4 sm:py-2", onClick: () => { resetForm(); setEditing(null); setAddOpen(true); }, children: "Add Supplier" }))] }), _jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Suppliers" }) }), _jsx(CardContent, { children: _jsx(Table, { columns: columns, data: suppliers, emptyText: "No suppliers" }) })] }), _jsxs(Card, { className: "mt-4", children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Monthly Summary" }) }), _jsx(CardContent, { children: _jsx(Table, { columns: monthlyColumns, data: monthlySummaries, emptyText: "No monthly summary data" }) })] }), _jsx(Modal, { open: addOpen, onClose: () => { if (!busy) {
                            setAddOpen(false);
                            setEditing(null);
                            resetForm();
                        } }, title: editing ? 'Edit Supplier' : 'Add Supplier', children: _jsxs("div", { className: "space-y-3", children: [_jsxs("div", { children: [_jsx(Input, { label: "Supplier Name", value: formName, onChange: (e) => setFormName(e.target.value), disabled: isTeamLeader }), errors.name && (_jsx("div", { className: "mt-1 text-xs text-[color:var(--danger)]", children: errors.name }))] }), _jsxs("div", { children: [_jsx(Input, { label: "Contact Person", value: formContact, onChange: (e) => setFormContact(e.target.value), disabled: isTeamLeader }), errors.contactPerson && (_jsx("div", { className: "mt-1 text-xs text-[color:var(--danger)]", children: errors.contactPerson }))] }), _jsx("div", { children: _jsx(Input, { label: "Phone", value: formPhone, onChange: (e) => setFormPhone(e.target.value), disabled: isTeamLeader }) }), !isTeamLeader && (_jsxs("div", { className: "flex justify-end gap-2 pt-2", children: [_jsx(Button, { variant: "secondary", onClick: () => { if (!busy) {
                                                setAddOpen(false);
                                                setEditing(null);
                                                resetForm();
                                            } }, disabled: busy, children: "Cancel" }), _jsx(Button, { onClick: handleCreateSupplier, disabled: busy, children: "Save" })] }))] }) })] })] }));
}
