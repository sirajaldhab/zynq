...
import { bulkCreateGeneralExpenses, deleteGeneralExpense, fetchGeneralExpenses, GeneralExpenseDto, updateGeneralExpense } from '../../api/financeService';
...
export default function GeneralExpensePage() {
  ...
  const [rows, setRows] = React.useState<GeneralRow[]>([]);
  const [sort, setSort] = React.useState<SortState | null>(null);
  const [page, setPage] = React.useState(1);
  const pageSize = 20;
  const [loading, setLoading] = React.useState(false);
  const [editRow, setEditRow] = React.useState<GeneralRow | null>(null);
  ...
  const loadExisting = React.useCallback(async () => {
    ...
    try {
      ...
      const data = ((res as any).rows ?? (res as any).data ?? []) as GeneralExpenseDto[];
      const mapped: GeneralRow[] = data.map((e, idx) => {
        const note = e.note || '';
  ...
  const sortedRows = React.useMemo(() => {
    ...
    const key = String(sort.key) as keyof GeneralRow;
  ...
  const columns: AdvColumn<GeneralRow>[] = [
  ...
  async function handleConfirmImport(rowsData: Record<string, any>[]) {
    if (!rowsData.length) {
  ...
  async function handleConfirmImportWithCompany() {
    ...
    const mapped: GeneralRow[] = pendingImportRows.map((r, idx) => {
    ...
    try {
      await bulkCreateGeneralExpenses(
        mapped.map((m) => ({
          date: m.date,
    ...
    <IonPage>
      ...
      <IonContent className="bg-[color:var(--bg)] py-8">
        <div className="space-y-4 px-4 sm:px-6 lg:px-8">
          <div className="text-lg font-semibold">Finance / General Expense</div>
          <div className="zynq-muted text-sm">Home &gt; Finance &gt; General Expense</div>
          <div>
            <Button
              variant="secondary"
              size="sm"
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="text-2xl font-semibold">
              {summaryTotal.toLocaleString(undefined, {
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="mt-2 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <Input
                label="Search"
                placeholder="Search general expenses"
                value={search}
        ...
        <BulkImportModal
          ...
          templateHint="Expected columns: S/N, Date, Inv No, Description, Supplier, Unit, Qty., Total Amount"
        />
...
function EditGeneralExpenseModal({ row, companies, onClose, onSubmit }: { row: GeneralRow; companies: DocumentCompanyDto[]; onClose: () => void; onSubmit: (updated: GeneralRow) => void | Promise<void> }) {
  const [form, setForm] = React.useState<GeneralRow>(row);
  React.useEffect(() => { setForm(row); }, [row]);

  function update<K extends keyof GeneralRow>(key: K, value: GeneralRow[K]) {
  ...
  return (
    <Modal open={true} onClose={onClose} title="Edit General Expense">
